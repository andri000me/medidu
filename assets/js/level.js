function Level(){
			...
			dataType: 'json',
				...
				{					
					if (html == true) { 
						...
						this.url = "index.php/Control_main/openPage/level";
    ...
    $('#example tbody').on('click','tr', function () {
        ...
        if (table.rows('.selected').data().length == 0) {
        	...
        	level.setRowData(level.getIndex(), $(this).closest('tr').attr('id'));
        	level.setIndex(level.getIndex()+1);
        }
    });

     $('#deleteRow').click(function(){
	    hideAll();
		level.setMaxLoad(100/table.rows('.selected').data().length);
	    level.getPage("Loading", "md");
	    level.setDataUrl("index.php/Control_level/delete/");
	    level.setDataAlert("deleted");
    	for (var i = 0; i < level.getIndex(); i++) {
	    	level.setDataString("id="+level.getRowData(i)+"&table=tbl_level&key=id");
	    	level.proses("POST", "deleteRow");
    	}
    });
	
	$('.ajax').click(function(e){
		e.preventDefault();
		level.setDataUrl($(this).attr('href'));
		level.proses("GET", "NO");
	});	
    ...
    $("#validate").submit(function(e) {
        ...
        if($("#operasi").val() == "create"){
            level.setDataUrl("index.php/Control_level/insert");      
            level.setDataString($("#validate").serialize()+"&table=tbl_level");    
            level.setDataAlert(" saved data");    
        }else if($("#operasi").val() == "update"){      
            level.setDataUrl("index.php/Control_level/update");      
            level.setDataString($("#validate").serialize()+"&table=tbl_level&key=id");
            level.setDataAlert(" update data");    
        }else if($("#operasi").val() == "delete"){
            level.setDataUrl("index.php/Control_level/delete");      
            level.setDataString($("#validate").serialize()+"&table=tbl_level&key=id");    
            level.setDataAlert(" deleted data");    
        }
            level.proses("POST", "NO_delete");   
        }
    });    
